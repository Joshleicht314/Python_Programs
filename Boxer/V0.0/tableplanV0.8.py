...
@dataclass
class Settings:
    height_steps:     int           = 2     # vertical sub-divisions per row unit
    width_steps:      int           = 1     # horizontal sub-divisions per column
    zoom_h:           float         = 1.0
    zoom_w:           float         = 1.0
...
class GridWidget(Widget):
    ...
    def _all_blocks_at(self, abs_step: int, ci: int) -> list[Block]:
        """Return all blocks at (abs_step, ci), accounting for wrap if enabled."""
        ...
        for b in self.table.blocks:
            ...
            if not wrap:
                # Normal: check direct overlap
                if bc <= ci < bc + b.width:
                    if bs <= abs_step < be:
                        (transp if b.transparent else solid).append(b)
            else:
                # Wrap mode: a block that extends past the bottom of column bc
                # continues from row 0 in column bc+1, etc.
                total_steps = be - bs          # total vertical steps in block
                rows_per_col = nr * hs         # steps per column before wrap
                # Iterate over each column segment of the block
                col_offset = 0
                remaining  = total_steps
                cur_row    = bs
                while remaining > 0:
                    cur_col = bc + col_offset
                    if cur_col >= nc:          # past end of table, stop
                        break
                    # How many rows fit in this column starting from cur_row
                    avail = rows_per_col - cur_row
                    seg   = min(remaining, avail)
                    seg_start = cur_row
                    seg_end   = cur_row + seg
                    if cur_col == ci and seg_start <= abs_step < seg_end:
                        (transp if b.transparent else solid).append(b)
                        break
                    remaining  -= seg
                    cur_row     = 0            # next column starts at row 0
                    col_offset += 1
    ...
    def _in_bounds(self, b: Block, step: int, ci: int) -> bool:
        ...
        if self.settings.block_wrap:
            # In wrap mode: block must start within table; overflow wraps to next col
            # Block is valid as long as it starts within the grid and there's
            # enough total column space to hold the wrapped content
            if not (0 <= ci < nc and 0 <= step < nr * hs):
                return False
            # Check we don't run out of columns when wrapping
            total_steps  = bh
            rows_per_col = nr * hs
            avail_first  = rows_per_col - step
            extra_steps  = total_steps - avail_first
            if extra_steps > 0:
                extra_cols_needed = math.ceil(extra_steps / rows_per_col)
                return ci + 1 + extra_cols_needed <= nc
            return True
    ...
    def _has_conflict(self, b: Block, step: int, ci: int) -> bool:
        ...
        for ob in self.table.blocks:
            ...
            ocs, oce = round(float(ob.col) * 1), round(float(ob.col) + ob.width)
            bc, bce  = ci, ci + b.width
            if bc >= oce or bce <= ocs: continue
    ...
    def _get_conflict_ids(self) -> set[int]:
        ...
        for i in range(len(solid)):
            for j in range(i + 1, len(solid)):
                ...
                ac, ace = float(a.col), float(a.col) + a.width
                bc, bce = float(b.col), float(b.col) + b.width
                if ac >= bce or bc >= ace: continue
        ...
        for i, ci in enumerate(vis_cols):
            ...
            for ghost_block, gs, gc in ghosts:
                ...
                if gs <= abs_step < ge and gc <= ci < gce:
                    ...
                    line_abs = (abs_step - gs) * step_h + line_in_step
            ...
            if not cell_done:
                ...
                if all_blks:
                    ...
                    if self.flash_mode:
                        ...
                        line_abs = (abs_step - bs) * step_h + line_in_step
    ...
    def _cmd_shelf_pull(self) -> None:
        ...
        # Enter shelf-pick mode — show register list, pick by digit
        self._shelf_pick_mode = True
        self.refresh()

    def _render_shelf_picker(self, W: int):
        """Render the shelf picker like vim :reg — shown in status bar."""
        lines = []
        for i, entry in enumerate(self.shelf[:9]):
            names = ", ".join(b.name for b in entry[:3])
            if len(entry) > 3: names += f" +{len(entry)-3}"
            lines.append(f" {i}: [{names}]")
        msg = "  SHELF: " + "  ".join(lines) + "  │ press 0-9 to pick  Esc cancel"
        from rich.segment import Segment
        from rich.style import Style
    ...
    def _cmd_transpose(self) -> None:
        ...
        for b in self.table.blocks:
            ...
            b.width  = max(1, round(old_h))
        self.cursor_row = self.cursor_col * hs
        self.cursor_col = min(len(self.table.columns) - 1, self.cursor_row // hs)
        self.view_row_off = 0; self.view_col_off = 0
        self._invalidate_conflicts()
        self.status = (f"  Transposed \u2014 {len(self.table.rows)} rows \xd7 "
                       f"{len(self.table.columns)} cols  (T again to restore)")
