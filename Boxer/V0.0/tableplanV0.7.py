...
class GridWidget(Widget):
    ...
    def _cmd_shelf_pull(self) -> None:
        if not self.shelf:
            self.status = "  Shelf is empty."; self.status_err = True; return
        if len(self.shelf) == 1:
            self._shelf_place(0); return
        # Show numbered list
        items = "; ".join(
            f"{i}: [{', '.join(b.name for b in entry[:2])}{'...' if len(entry)>2 else ''}]"
            for i, entry in enumerate(self.shelf)
        )
        def done(vals: list[str]) -> None:
            try: idx = int(vals[0])
            except ValueError:
                self.status = "  Invalid index."; self.status_err = True; return
            if 0 <= idx < len(self.shelf):
                self._shelf_place(idx)
            else:
                self.status = "  Index out of range."; self.status_err = True
        self._prompt([f"Shelf ({items}) — enter index"], done)

    def _shelf_place(self, idx: int) -> None:
        entry  = self.shelf.pop(idx)
        hs     = self.settings.height_steps
        # Place relative to cursor — use first block's original position
        if entry:
            orig_row = entry[0].row; orig_col = entry[0].col
            dr = self.cursor_row / hs - orig_row
            dc = self.cursor_col - int(orig_col)
            self._snapshot()
            for b in entry:
                b.row = max(0.0, b.row + dr)
                b.col = max(0, int(b.col) + dc)
