#!/usr/bin/env python3
"""
tableplan v0.5  —  vim-style terminal table organizer
...
╠═══════════════════════════════════║  "       open in $EDITOR     ║
║  :set wrap / nowrap               ║  ?       this help           ║
║  :set width N   visible cols      ║  Tab     autocomplete cmd    ║
║  :set height N  visible rows      ╠══════════════════════════════╣
║  :set tolerance N  steps/unit     ║  Overlap → always red        ║
║  :set home  reset zoom/view       ║  :check  list overlapping    ║
╚═══════════════════════════════════╩══════════════════════════════╝
...
_CMD_COMPLETIONS = sorted([
    ":check", ":q", ":q!", ":w", ":wq", ":x",
    ":set home", ":set nowrap", ":set wrap",
    ":set width ", ":set height ", ":set tolerance ",
])

_HINT = ("  hjkl:move  spc:grab  a:add  e:edit  x:del  v:transp  V:visual  "
    ...
    def render_line(self, y: int) -> Strip:  # noqa: C901
        ...
        if y == 0:
            ...
            for i, ci in enumerate(vis_cols):
                ...
                segs.append(Segment(
                    self.table.columns[ci][:col_widths[i]].center(col_widths[i]), S_HEADER))
            segs.append(Segment("│", S_BORDER))
        ...
        for i, ci in enumerate(vis_cols):
            ...
            if not cell_done:
                ...
                if block is not None:
                    ...
                    raw_txt = block.name
                    # Selection indicator
                    if id(block) in self.selected_ids:
                        raw_txt = "►" + block.name
                    txt = raw_txt if lib == max(0, tot // 2) else ""
                    if is_cursor:
                        style = _cursor_on(block.color_idx)
                    elif id(block) in self.selected_ids:
                        style = _selected_style(block.color_idx)
